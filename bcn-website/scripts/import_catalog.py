...
def build_products(square_rows: list[dict[str, str]], etsy_rows: list[dict[str, str]]) -> list[dict[str, Any]]:
    ...
    for index, (name, rows) in enumerate(grouped.items()):
        ...
        product: dict[str, Any] = {
            ...
            "shippingClass": "",
            "shippingEnabled": any(truthy(row.get("Shipping Enabled", "")) for row in rows),
            "localPickupEnabled": any(truthy(row.get("Pickup Enabled", "")) for row in rows),
            "packedWeightOz": None,
            "packedLengthIn": None,
            "packedWidthIn": None,
            "packedHeightIn": None,
            "shipsAlone": False,
            "expeditedRequired": False,
            "allowGroundAdvantage": True,
            "freeShippingEligible": False,
            "shippingSurchargeCents": 0,
            "maxQuantityPerPackage": 1,
            "preferredPackageId": None,
            "shippingConfigurationComplete": False,
            "tags": tags,
