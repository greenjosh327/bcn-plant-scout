...
async function requireAdmin(request: Request, supabase: ReturnType<typeof getSupabaseServiceClient>) {
...
async function selectOrderWithItems(supabase: ReturnType<typeof getSupabaseServiceClient>, orderId: string) {
  const { data, error } = await supabase
    .from("orders")
    .select(`
      *,
